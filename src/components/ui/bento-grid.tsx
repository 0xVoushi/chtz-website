...
export function BentoGrid({ children, className }: BentoGridProps) {
  return (
    <div
      className={cn(
        'grid grid-cols-1 md:grid-cols-6 lg:grid-cols-6 gap-0 rounded-xl border border-[var(--color-border-light)]',
        className
      )}
    >
      {children}
...
export function BentoCard({ id, children, className }: BentoCardProps) {
  return (
    <div
      id={id}
      className={cn(
        'relative overflow-hidden p-6 sm:p-8 border-[var(--color-border-light)]',
        className
      )}
    >
      {children}
...
export function BentoTitle({ children, className }: BentoTitleProps) {
  ...
  return (
    <h3
      className={cn(
        'text-left text-lg font-semibold tracking-tight text-[var(--color-near-black)]',
        className
      )}
    >
      {children}
...
export function BentoDescription({ children, className }: BentoDescriptionProps) {
  ...
  return (
    <p
      className={cn(
        'text-left text-sm font-normal text-[var(--color-muted)] mx-0 my-2',
        className
      )}
    >
      {children}
